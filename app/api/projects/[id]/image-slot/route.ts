 ...
 *
 * Body:
 *   field: 'appearance_images' | 'storyboard_images' | 'exploded_view_image'
 ...
 *   4. 写回 DB
 *
 * 防串台：projectId 从 URL params 强制注入，不信任 body
...
export async function PATCH(
  ...
  try {
    ...
    // 参数校验
    if (!field || !['appearance_images', 'storyboard_images', 'exploded_view_image'].includes(field)) {
      return NextResponse.json(
        { error: '无效的 field 参数，仅支持 appearance_images、storyboard_images 或 exploded_view_image' },
        { status: 400 }
      );
    }
    ...
    // Supabase 动态字段选择导致 TS 无法推断类型，使用 any 绕过
    ...
    if (field === 'exploded_view_image') {
      const prev = (projectData.exploded_view_image && typeof projectData.exploded_view_image === 'object')
        ? (projectData.exploded_view_image as unknown as Record<string, unknown>)
        : {};
      const patch = {
        projectId,
      ...
      const { data: updatedExploded, error: explodedErr } = await supabaseAdmin
        .from('projects')
        .update({ exploded_view_image: patch })
        .eq('id', projectId)
        .eq('user_id', user.id)
        .select()
        .single();
      if (explodedErr) {
        console.error('更新爆炸图槽位失败:', explodedErr);
        return NextResponse.json({ error: `更新失败: ${explodedErr.message}` }, { status: 500 });
      }
      return NextResponse.json(updatedExploded);
    }

    // 读取当前数组，只替换指定槽位
    const maxSlots = field === 'appearance_images' ? 3 : 6;
    const currentArray = Array.isArray(projectData[field])
      ? [...(projectData[field] as any[])]
      : [];
    ...
    if (field === 'storyboard_images') {
      // storyboard_images 数组中每项是对象 —— 原子合并
    ...
    } else if (field === 'appearance_images') {
      // appearance_images 数组中每项现在也是对象（向后兼容旧字符串数据）
      const prev = (currentArray[index] && typeof currentArray[index] === 'object')
    ...
    // 原子写回（仅 appearance_images / storyboard_images）
