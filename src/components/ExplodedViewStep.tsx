...
export default function ExplodedViewStep({ image, isLoading, idea, projectId, referenceImage, onUpdate, onGeneratingChange, onGenerated }: ExplodedViewStepProps) {
  ...
  const isGenerating = internalIsGenerating;

  const setProjectGenerating = useCallback((value: boolean) => {
    setInternalIsGenerating(value);
  }, []);

  useEffect(() => {
    onGeneratingChange?.(internalIsGenerating);
  }, [internalIsGenerating, onGeneratingChange]);

  // useImageJobs 每 2s 轮询：同步生成中状态 + 合并已完成图片 URL
  useImageJobs({
    projectId, step: 'exploded_view',
    onJobsLoaded: useCallback((slots: Record<number, boolean>, completedImages: Record<number, string>) => {
      // 合并式更新 + 本地锁保护
      setInternalIsGenerating(prev => {
        if (localPendingRef.current) return true;
        if (slots[0]) return true;
        if (completedImages[0]) { localPendingRef.current = false; return false; }
        return prev;
      });
  ...
  const generateImage = async () => {
    ...
    try {
      ...
      const body: any = { type: 'exploded_view', prompt: [
      ...
      ].join(', '), slotIndex: 0, expectedTotal: 1, userId: user?.id, projectId };
      if (referenceImage) body.referenceImage = referenceImage;
      const response = await fetch('/api/image', { method: 'POST', headers: { 'Content-Type': 'application/json' }, body: JSON.stringify(body) });
      const data = await response.json();
      if (!response.ok || data.error) throw new Error(data.error || `HTTP ${response.status} (${response.statusText || '未知错误'})`);
      if (data.imageUrl) { onUpdate(data.imageUrl); completedImmediately = true; setLastError(null); }
    } catch (error: any) {
    ...
    if (completedImmediately) { localPendingRef.current = false; setProjectGenerating(false); }
  };
    ...
    <div className={`${stepSubCardClass} p-6 sm:p-8`}>
      ...
      <div className="grid grid-cols-1 lg:grid-cols-5 gap-6">
        ...
        <div className="lg:col-span-3">
          {image ? (
            <div className="group relative overflow-hidden rounded-[1.75rem] border border-rose-100 bg-white shadow-lg shadow-rose-100/20">
            ...
            <div className="flex aspect-[4/3] flex-col items-center justify-center gap-4 rounded-[1.75rem] border-2 border-dashed border-slate-200 bg-gradient-to-b from-slate-50 to-white px-6">
              ...
              <button onClick={generateImage} disabled={isGenerating} className={stepPrimaryButtonClass}>
                {isGenerating ? (<><span className="h-4 w-4 animate-spin rounded-full border-2 border-white border-b-transparent" />生成中…</>) : (<><svg className="h-4 w-4" fill="none" stroke="currentColor" viewBox="0 0 24 24"><path strokeLinecap="round" strokeLinejoin="round" strokeWidth={2} d="M13 10V3L4 14h7v7l9-11h-7z" /></svg>生成爆炸图</>)}
              </button>
            </div>
          )}
        </div>
