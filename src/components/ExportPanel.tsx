...
const SECTIONS = [
  ...
  { key: 'product_intro', label: '产品介绍', icon: 'M13 10V3L4 14h7v7l9-11h-7z', accent: 'bg-indigo-50 text-indigo-600 border-indigo-200', dot: 'bg-indigo-500' },
  ...
  { key: 'cmf', label: 'CMF 方案', icon: 'M7 21a4 4 0 01-4-4V5a2 2 0 012-2h4a2 2 0 012 2v12a4 4 0 01-4 4zm0 0h12a2 2 0 002-2v-4a2 2 0 00-2-2h-2.343M11 7.343l1.657-1.657a2 2 0 012.828 0l2.829 2.829a2 2 0 010 2.828l-8.486 8.485M7 17h.01', accent: 'bg-rose-50 text-rose-600 border-rose-200', dot: 'bg-rose-500' },
  { key: 'storyboard', label: '故事板', icon: 'M15 10l4.553-2.276A1 1 0 0121 8.618v6.764a1 1 0 01-1.447.894L15 14M5 18h8a2 2 0 002-2V8a2 2 0 00-2-2H5a2 2 0 00-2 2v8a2 2 0 002 2z', accent: 'bg-amber-50 text-amber-600 border-amber-200', dot: 'bg-amber-500' },
...
export default function ExportPanel({ project, selectedAppearance }: ExportPanelProps) {
  ...
  const handleExport = async (type: 'ppt' | 'pdf') => {
    ...
    try {
      ...
      const response = await fetch(`/api/export/${type}`, {
        ...
        body: JSON.stringify({ projectId: project.id, userId: user?.id, selectedAppearance, includeSections }),
      });
  ...
  return (
    <div className={`${stepSubCardClass} p-6 sm:p-8`}>
      ...
      {/* Error */}
      {error && (
        <div className="mb-8 rounded-2xl border border-red-200 bg-gradient-to-br from-red-50 to-red-50/50 px-5 py-4 text-sm font-semibold text-red-700 flex items-center gap-3 shadow-sm">
          <div className="flex h-9 w-9 shrink-0 items-center justify-center rounded-xl bg-red-100">
            <svg className="h-5 w-5 text-red-500" fill="none" stroke="currentColor" viewBox="0 0 24 24"><path strokeLinecap="round" strokeLinejoin="round" strokeWidth={2} d="M12 8v4m0 4h.01M21 12a9 9 0 11-18 0 9 9 0 0118 0z" /></svg>
          </div>
          <span>{error}</span>
        </div>
      )}
        ...
        <div className="mb-4 flex items-center justify-between">
          <div className="flex items-center gap-2.5">
            <div className="flex h-8 w-8 items-center justify-center rounded-xl bg-slate-100">
              <svg className="h-4 w-4 text-slate-600" fill="none" stroke="currentColor" viewBox="0 0 24 24"><path strokeLinecap="round" strokeLinejoin="round" strokeWidth={2} d="M9 5H7a2 2 0 00-2 2v12a2 2 0 002 2h10a2 2 0 002-2V7a2 2 0 00-2-2h-2M9 5a2 2 0 002 2h2a2 2 0 002-2M9 5a2 2 0 012-2h2a2 2 0 012 2" /></svg>
            </div>
            <h3 className="text-base font-black text-slate-900">选择导出内容</h3>
            ...
            <button
              ...
              className={`group relative flex flex-col items-center gap-2 rounded-2xl border-2 p-3.5 text-center transition-all duration-200 select-none ${
                !item.hasContent
                  ...
                  : 'border-slate-100 bg-white hover:border-slate-200 hover:shadow-sm hover:-translate-y-0.5'
              }`}
            >
              {/* Status dot */}
              {item.hasContent && (
                <div className={`absolute top-2 right-2 h-2 w-2 rounded-full transition ${
                  includeSections[item.key] ? item.dot : 'bg-slate-300'
                }`} />
              )}
              {/* Icon */}
      ...
      <div className="grid grid-cols-1 gap-5 md:grid-cols-2">
        {/* PPT Card */}
        <div className="group relative overflow-hidden rounded-[1.75rem] border border-blue-100 bg-gradient-to-br from-blue-50/60 via-white to-white p-7 shadow-lg shadow-blue-100/20 transition hover:shadow-xl hover:shadow-blue-100/30 hover:-translate-y-0.5">
            ...
            <div className="space-y-2 mb-6">
              {[
                ...
                <div key={i} className="flex items-center gap-2 text-xs font-medium text-slate-600">
                  <svg className="h-3.5 w-3.5 shrink-0 text-blue-400" fill="none" stroke="currentColor" viewBox="0 0 24 24"><path strokeLinecap="round" strokeLinejoin="round" strokeWidth={2} d={f.icon} /></svg>
                  {f.text}
                </div>
              ))}
            </div>
